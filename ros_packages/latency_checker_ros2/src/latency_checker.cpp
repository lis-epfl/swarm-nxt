...
namespace latency_checker
{
  ...
  LatencyChecker::LatencyChecker() : ::rclcpp::Node("latency_checker")
  {
    ...
    this->declare_parameter<std::string>("config_file_path", "config/peer_list.yaml");
  ...
  void LatencyChecker::HandleHeartbeatMessage(const latency_checker_ros2::msg::Heartbeat &msg)
  {
...
void LatencyChecker::PublishHeartbeat()
{
  ...
  for (const auto &entry : heartbeat_map_)
  {
    ...
    latency_msg.latency = rclcpp::Duration::from_nanoseconds(latency.to_chrono<std::chrono::nanoseconds>().count());
